<template>
  <a-layout>
    <!-- 左侧部分开始 -->
    <a-layout-sider :style="{ overflow: 'auto', height: '100vh', background: theme == 'light' ? '#fff' : '#001628' }"
                    v-model:collapsed="sideCollapsed">
      <!-- 左侧logo开始 -->
      <div class="logo">
        <img src="/vite.svg" class="logo_img" alt="Vite logo"/>
        <span class="logo_text" :style="{ color: theme == 'light' ? '#011528' : '#fff' }"
              v-show="!sideCollapsed">知识库</span>
      </div>
      <!-- 左侧logo结束 -->
      ...
      <div style="padding-left: 12px">
        <template v-for="(item, index) in menuList">
          <div :class="menuIndex == item.key ? 'silder-item silder-item-active' : 'silder-item'"
            ...
            <div class="title">
              <!-- 如果是“文件审核”，显示特殊图标和标题 -->
              <template v-if="item.title === '文件审核'">
                <ZhihuOutlined class="menu-icon" />
                <span v-if="!sideCollapsed">{{ item.title }}</span>
              </template>

              <template  v-else-if="item.title === '系统设置'">
              ...
              <template v-else>
                <svg-icon
                    :name="item.icon"
                    class="menu-icon"
                    :color="menuIndex === item.key ? '#1e6fff' : ''"
                />
                <span v-if="!sideCollapsed">{{ item.title }}</span>
              </template>
            </div>
          ...
          <div :class="!item.isExpand ? 'list-box' : 'list-box list-box-open'"
               v-if="!sideCollapsed && item.children && item.children.length > 0">
            <div class="list-items">
              <div :class="menuIndex == child.key ? 'list-item silder-item-active' : 'list-item'"
                   v-for="(child, childIndex) in item.children" @click="toPath(child, childIndex, index, true)">
                <svg-icon :name="item.icon" class="menu-icon"
                          :color="menuIndex == child.key ? '#1e6fff' : ''"></svg-icon>
    ...
    <a-layout>
      <!-- 右侧header开始 -->
      <a-layout-header
          style="background: #fff; padding: 0; height: 50px; display: flex; align-items: center; position: relative">
